#!/usr/bin/env python3
"""
Evaluate a chat-style Vision-Language model (e.g., lmms-lab/LLaVA-OneVision-1.5-8B-Instruct)
...
def _device_map_from_str(device: str):
    ...
    if d.startswith("cuda"):
        # Default to GPU 0 if no index; otherwise parse ":idx"
        idx = 0
        if ":" in d:
            try:
                idx = int(d.split(":", 1)[1])
...
def load_model_and_processor(model_id: str, device: str):
    """
    Loads either a CausalLM (e.g., LLaVA-OneVision) or Vision2Seq (Qwen-style) model,
    along with its AutoProcessor. Picks dtype based on device capabilities.
    """
    ...
    last_err = None
    model = None
    for Cls in (AutoModelForCausalLM, AutoModelForVision2Seq):
        try:
...
def main():
    ...
    ap.add_argument(
        "--model-id", default="lmms-lab/LLaVA-OneVision-1.5-8B-Instruct",
        help="HF model id (e.g., lmms-lab/LLaVA-OneVision-1.5-8B-Instruct)"
    )
    ap.add_argument(
