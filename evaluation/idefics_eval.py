#!/usr/bin/env python3
"""
Evaluate Idefics3-8B on an annotated pairs dataset (A.jpg labeled, B.jpg augmented).

For each pair:
  - Send BOTH images (A first, B second) with a prompt to HuggingFaceM4/Idefics3-8B-Llama3.
...
def build_messages(prompt: str) -> List[Dict[str, Any]]:
    # For Idefics3 we pass image *placeholders* in the chat, and supply actual PIL images separately
    strict = (
        f"{prompt}\n\n"
    ...
    messages = [
        {
            "role": "user",
            "content": [
                {"type": "image"},  # A (placeholder)
                {"type": "image"},  # B (placeholder)
...
def load_model_and_processor(model_id: str, device: str):
    ...
    # You can customize image resolution via processor kwargs if needed:
    # processor = AutoProcessor.from_pretrained(model_id, size={"longest_edge": 4*364})
...
def run_inference(
    ...
    for it in tqdm(items, desc=f"{device}"):
        ...
        # 1) Build chat with two image placeholders
        messages = build_messages(prompt)

        # 2) Convert chat to a text prompt string per Idefics3 recipe
        prompt_str = processor.apply_chat_template(
            messages,
            add_generation_prompt=True
        )

        # 3) Let the processor build tensors; pass actual images in A-then-B order
        inputs = processor(
            text=prompt_str,
            images=[imgA, imgB],
            return_tensors="pt"
        ...
        # Cache input length to slice out only the generated continuation (optional)
        ...
        # Slice to decode only new tokens (works across batch size 1)
        gen_ids = outputs[0, input_len:]
        # For consistency, use batch_decode API (still fine with a single sample)
...
def main():
    ap = argparse.ArgumentParser(
        description="Evaluate Idefics3-8B on annotated pairs (A,B)."
    )
    ap.add_argument(
    ...
    ap.add_argument(
        "--model-id", default="HuggingFaceM4/Idefics3-8B-Llama3",
        help="HF model id for Idefics3"
    )
    ap.add_argument(
